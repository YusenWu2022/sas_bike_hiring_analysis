...
/* 打印合并后的数据集，查看Minnehaha Park站点每小时的出站和入站总数 */
proc print data=hennepin_count;
run;

/* 绘制Minnehaha Park站点每小时的出站和入站数量折线图 */
...
/* 计算每小时的共享单车数量差值（出站数减去入站数），并累加计算总的共享单车短缺数量 */
data hennepin_count;
    set hennepin_count;
    if Hour = 0 then do;
...
/* 打印每小时的共享单车数量差值和累计短缺数量 */
proc print data=hennepin_count;
    var diff;
    var sum_diff;
run;

/* 绘制每小时的共享单车数量差值和累计短缺数量折线图 */
proc sgplot data=hennepin_count;
